  ...
  Future<void> initSocket() async {
    try {
      ...
      socket.on('position-change', (data) {
        print('connected');
        var latlng = jsoncode(data);
        final GoogleMapController controller = await _controller.future;
        controller.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(
            target: LatLng(latlng['lat'], latlng['lng']), zoom: 19)));
            var image=await BitMapDescriptor.fromAssetImage(ImageConfiguration(size: Size(10,10)),'assets/origin.png');
            Marker marker=Marker(markerId: MarkerId('origin'),position: LatLng(latlng['lat'], latlng['lng']),icon: image);
            setState(() {
